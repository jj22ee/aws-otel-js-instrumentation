...
import {
  ...
  stampLocalEnvironment,
} from '../../../src/serviceevents/utils/environment-resolver';
...
describe('environment-resolver', function () {
  describe('resolveLocalEnvironment()', function () {
    ...
    it('EKS → eks:<cluster>/<namespace>', function () {
      expect(
        resolveLocalEnvironment({
          attributes: { 'cloud.platform': 'aws_eks', 'k8s.cluster.name': 'my-cluster', 'k8s.namespace.name': 'default' },
        })
    ...
    it('non-EKS Kubernetes → k8s:<cluster>/<namespace>', function () {
      expect(
        resolveLocalEnvironment({ attributes: { 'k8s.cluster.name': 'c', 'k8s.namespace.name': 'team-a' } })
      ).toBe('k8s:c/team-a');
    });
    ...
    it('EC2 with ASG → ec2:<asg>', function () {
      expect(
        resolveLocalEnvironment({ attributes: { 'ec2.tag.aws:autoscaling:groupName': 'my-asg' } })
      ).toBe('ec2:my-asg');
    });
