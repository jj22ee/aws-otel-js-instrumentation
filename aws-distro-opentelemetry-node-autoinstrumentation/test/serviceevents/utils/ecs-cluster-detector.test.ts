...
describe('EcsClusterDetector', function () {
  ...
  it('derives the cluster ARN from the task ARN when Cluster is a bare name', function (done) {
    startMetadata(
      (req, res) => {
        if (req.url === '/task') {
          res.writeHead(200);
  ...
  it('uses Cluster directly when it is already a full ARN', function (done) {
    startMetadata(
      (req, res) => {
        res.writeHead(200);
        res.end(
  ...
  it('sets cloud.platform=aws_ecs only when a cluster resolved', function (done) {
    startMetadata(
      (req, res) => {
        res.writeHead(200);
