...
describe('Ec2AutoScalingGroupDetector', function () {
  ...
  it('fetches the ASG from IMDS instance tags (token + tag value)', function (done) {
    startImds((req, res) => {
  ...
  it('returns undefined when instance tags are not enabled (404 on tag path)', function (done) {
    startImds((req, res) => {
      if (req.method === 'PUT' && req.url === '/latest/api/token') {
        res.writeHead(200);
        res.end('fake-token');
