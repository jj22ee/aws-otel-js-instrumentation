...
try {
  ...
  // TEST-ONLY: this resource carries the ASG tag that is intentionally kept off the global
  // resource; off-EC2 it equals configuration.resource.
  ...
  try {
    ...
    if (serviceeventsIsMainThread && decision.action === 'skipForceEnabledWithoutEndpoints') {
      ...
      const serviceeventsConfig = {
        ...
        enabled: true,
        // TEST-ONLY: ServiceEvents resource (global + EC2 ASG detector); off-EC2 equals
