...
import { diag, DiagConsoleLogger, Attributes, AttributeValue, TimeInput, ROOT_CONTEXT, SpanContext, HrTime } from "@opentelemetry/api"
import { LoggerProvider } from "@opentelemetry/sdk-logs";
...
export class LLOHandler {
    ...
    private _prompt_content_pattern = new RegExp("^gen_ai\\.prompt\\.(\\d+)\\.content$");
    private _completion_content_pattern = new RegExp("^gen_ai\\.completion\\.(\\d+)\\.content$");
    private _openinference_input_msg_pattern = new RegExp("^llm\\.input_messages\\.(\\d+)\\.message\\.content$");
    private _openinference_output_msg_pattern = new RegExp("^llm\\.output_messages\\.(\\d+)\\.message\\.content$");
    ...
    */
    ...
    public process_span_events(span: ReadableSpan) : void {
        ...
        */
        ...
        for (const event of span.events) {
            ...
            let updated_event_attributes = this._filter_attributes(event.attributes)
    ...
    private _emit_llo_attributes(
        span: ReadableSpan, attributes: Attributes, event_timestamp: HrTime | undefined = undefined
    ) {
        /*
    ...
    private _filter_attributes(attributes: Attributes) : Attributes {
        ...
        */
        ...
        let filtered_attributes: Attributes = {}
    ...
    private _is_llo_attribute(key: string) : boolean {
        ...
        */
        ...
        if (key in this._exact_match_patterns) {
            return true
        }

        // Then check regex patterns
        for (const pattern of this._regex_patterns) {
    ...
    private _extract_gen_ai_prompt_events(
        ...
        */
        ...
        for (const key in attributes) {
            if (key.match(this._prompt_content_pattern)) {
        ...
        let events: Event[] = []
        let span_ctx = span.spanContext();
        let gen_ai_system = span.attributes["gen_ai.system"] || "unknown"

        // Use helper method to get appropriate timestamp (prompts are inputs)
        let prompt_timestamp = this._get_timestamp(span, event_timestamp, true)

        // Find all prompt content attributes and their roles
        let prompt_content_matches: PromptContent[] = []
        for (const [key, value] of Object.entries(attributes)) {
            let match = key.match(this._prompt_content_pattern)
            if (match) {
                let index = match[1]
                let role_key = `gen_ai.prompt.${index}.role`
                let role = attributes[role_key] || "unknown"
        ...
        for (const {key, value, role} of prompt_content_matches) {
            let event_attributes = {"gen_ai.system": gen_ai_system, "original_attribute": key}
            let body = {"content": value, "role": role}

            // Use helper method to determine event name based on role
            let event_name = this._get_event_name_for_role(role, gen_ai_system)

            let event = this._get_gen_ai_event(
                event_name,
                span_ctx,
                prompt_timestamp,
                event_attributes,
    ...
    private _extract_gen_ai_completion_events(
        ...
        */
        ...
        for (const key in attributes) {
            if (key.match(this._completion_content_pattern)) {
        ...
        let completion_timestamp = this._get_timestamp(span, event_timestamp, false)

        // Find all completion content attributes and their roles
        let completion_content_matches: PromptContent[] = []
        for (const [key, value] of Object.entries(attributes)) {
            let match = key.match(this._completion_content_pattern)
            if (match) {
                let index = match[1]
                let role_key = `gen_ai.completion.${index}.role`
                let role = attributes[role_key] || "unknown"
    ...
    private _extract_traceloop_events(
        ...
        */
        ...
        let gen_ai_system = span.attributes["traceloop.entity.name"] || "unknown"

        // Use helper methods to get appropriate timestamps
        let input_timestamp = this._get_timestamp(span, event_timestamp, true)
        let output_timestamp = this._get_timestamp(span, event_timestamp, false)
        ...
        for (const traceloop_attr of traceloop_attrs) {
            ...
            if (attr_key in attributes) {
                let event_attributes = {"gen_ai.system": gen_ai_system, "original_attribute": attr_key}
                let body = {"content": attributes[attr_key], "role": role}

                // Custom event name for Traceloop (always use system-specific format)
                let event_name = `gen_ai.${gen_ai_system}.message`
        ...
        let crewai_gen_ai_system = span.attributes["gen_ai.system"] || gen_ai_system;
        ...
        for (const crewai_attr of crewai_attrs) {
            ...
            if (attr_key in attributes) {
                let event_attributes = {"gen_ai.system": crewai_gen_ai_system, "original_attribute": attr_key}
                let body = {"content": attributes[attr_key], "role": role}

                // For CrewAI outputs, use the assistant message event
                let event_name = GEN_AI_ASSISTANT_MESSAGE
    ...
    private _extract_openlit_span_event_attributes(
        ...
        */
        ...
        let openlit_event_attrs = [
            {
                attr_key: OPENLIT_PROMPT, timestamp: prompt_timestamp, role: ROLE_USER
            }, // Assume user role for direct prompts
            {
                attr_key: OPENLIT_COMPLETION, timestamp: completion_timestamp, role: ROLE_ASSISTANT
            }, // Assume assistant role for completions
            {
                attr_key: OPENLIT_REVISED_PROMPT, timestamp: prompt_timestamp, role: ROLE_SYSTEM
            }, // Assume system role for revised prompts
            {
                attr_key: OPENLIT_AGENT_ACTUAL_OUTPUT, timestamp: completion_timestamp, role: ROLE_ASSISTANT
            }, // Assume assistant role for agent output
            {
                attr_key: OPENLIT_AGENT_HUMAN_INPUT, timestamp: prompt_timestamp, role: ROLE_USER
            }, // Assume user role for agent human input
        ]
    ...
    private _extract_openinference_attributes(
        ...
        */
        ...
        let openinference_direct_keys = {OPENINFERENCE_INPUT_VALUE, OPENINFERENCE_OUTPUT_VALUE}
        ...
        for (const key in attributes) {
            if (key.match(this._openinference_input_msg_pattern)) {
                has_input_msgs = true;
                break;
            }
        }
        ...
        for (const key in attributes) {
            if (key.match(this._openinference_output_msg_pattern)) {
        ...
        let events: Event[] = []
        let span_ctx = span.spanContext();
        let gen_ai_system = span.attributes["llm.model_name"] || "unknown"
        ...
        for (const openinference_direct_attr of openinference_direct_attrs) {
            ...
            if (attr_key in attributes) {
                ...
                let event = this._get_gen_ai_event(
                    event_name, span_ctx, timestamp, event_attributes, body, span
                )
        ...
        let input_messages: PromptContent[] = []
        for (const [key, value] of Object.entries(attributes)) {
            let match = key.match(this._openinference_input_msg_pattern)
            if (match) {
                let index = match[1]
                let role_key = `llm.input_messages.${index}.message.role`
                let role = attributes[role_key] || ROLE_USER  // Default to user if role not specified
        ...
        for (const {key, value, role} of input_messages) {
            ...
            let event = this._get_gen_ai_event(
                event_name, span_ctx, input_timestamp, event_attributes, body, span
            )
        ...
        let output_messages: PromptContent[] = [];
        for (const [key, value] of Object.entries(attributes)) {
            let match = key.match(this._openinference_output_msg_pattern)
            if (match) {
                let index = match[1]
                let role_key = `llm.output_messages.${index}.message.role`
                let role = attributes[role_key] || ROLE_ASSISTANT;  // Default to assistant if role not specified
        ...
        for (const {key, value, role} of output_messages){
            ...
            let event = this._get_gen_ai_event(
                event_name, span_ctx, output_timestamp, event_attributes, body, span
            )
    ...
    private _get_gen_ai_event(name: string, span_ctx: SpanContext, timestamp: TimeInput, attributes: Attributes, body: AnyValue, span: ReadableSpan): Event {
        /*
        ...
        */
        ...
        //[]
        let customContext = ROOT_CONTEXT
